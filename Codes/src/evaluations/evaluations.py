...
def full_evaluation(generator, real_train_dl, real_test_dl, config, **kwargs):
    ...
    # Permutation test
    fake_data = loader_to_tensor(fake_loader(generator, num_samples=5000, n_lags=config.n_lags, batch_size=128, config=config))
    idx = torch.randint(real_data.shape[0], (10000,))
    real_data_X = real_data[idx[:-5000]]
    real_data_X1 = real_data[idx[-5000:]]
    
    power, type1_error = sig_mmd_permutation_test(real_data_X, real_data_X1, fake_data, 5)
