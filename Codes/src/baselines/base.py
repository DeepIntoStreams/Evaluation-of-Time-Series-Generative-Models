...
class BaseTrainer:
    ...
    def evaluate(self, x_fake, step):
        ...
        self.best_G_loss = self.losses_history['G_loss'][-1] if self.best_G_loss == None else self.best_G_loss
        if self.losses_history['G_loss'][-1] < self.best_G_loss:
            self.best_G = copy.deepcopy(self.G.state_dict())
            self.best_G_loss = self.losses_history['G_loss'][-1]
        wandb.log({'best_G_loss': self.best_G_loss}, step)
