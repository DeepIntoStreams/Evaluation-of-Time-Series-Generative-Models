...
class CVAE(GeneratorBase):
    ...
    def __init__(self, input_dim: int, output_dim: int, dim: int, level: int, latent_dim: int, augmentation: str = None, hidden_dim: int = 50, alpha: float = 0.2):
        ...
        self.input_dim = signatory.logsignature_channels(
            self.aug_dim, level)  # feat_dimension: logsignature dimension

        self.encoder = encoder_block(
            self.feat_dim, self.latent_dim, self.hidden_dim)
...
class encoder_block(nn.Module):
    ...
    def forward(self, X_in, cond):
        ...
        epsilon = torch.randn(
            [input_x.shape[0], self.latent_dim]).to(mean.device)
